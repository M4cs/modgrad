...
import { resolveLayers, blobBackground, flatBackground, } from "./mesh";
...
export function Gradient(props) {
    const { preset, variant: variantProp, angle = 135, grain = false, grainScale = 180, grainFrequency, grainOctaves, blur = 0, animate = false, interactive = false, opacity = 1, blendMode, fixed = false, zIndex, className, style, children, seed = 1, theme = "system", } = props;
    const resolvedTheme = useResolvedTheme(theme);
    const base = preset ? presets[preset] : undefined;
    const colors = pickTheme(props.colors ?? base?.colors, resolvedTheme) ?? defaultColors;
    const variant = variantProp ?? base?.variant ?? "mesh";
    const background = pickTheme(props.background ?? base?.background, resolvedTheme) ??
        (resolvedTheme === "light" ? "#f4f4f7" : "#11111a");
    const effAngle = props.angle ?? base?.angle ?? angle;
    const layers = useMemo(() => resolveLayers(colors, seed), 
    ...
    const speed = typeof animate === "object" && animate.speed ? animate.speed : 1;
    ...
    return (_jsxs("div", { className: className, style: container, "aria-hidden": true, children: [isLayered ? (_jsx("div", { style: {
                    position: "absolute",
                    ...
                    ...(blur
                        ...
                        : null),
                }, children: layers.map((l, i) => (_jsx("div", { ref: (el) => {
                        ...
                        willChange: (animateOn || interactive) && !blur ? "transform" : undefined,
                    } }, i))) })) : (_jsx("div", { style: {
                    ...FILL,
                    background: flatBackground(variant, layers, effAngle),
                    filter: blur ? `blur(${blur}px)` : undefined,
                    // zoom in so the blur's soft edge never reveals a dark border.
                    transform: blur ? "scale(1.4)" : undefined,
                } })), grainAmount > 0 && (_jsx("div", { style: {
                    ...FILL,
